...
{
$_SESSION['submittedalready']="आप पहले ही सबमिट कर चुके हैं!";?>
<div class="alert alert-warning">
<?php
echo $_SESSION['submittedalready'].'<br/>'; 

echo 'आपके इतने नंबर हैं .......' . $_SESSION['score'] . ' इतने मे से .......' . $_SESSION['x'] ;
}?>
</div>
<?php
$y=0;
...
{       
    ...
    $result =  $mysqli->query("SELECT * FROM mcq WHERE id=$questionId") or die($mysqli->error);
    ...
    echo 'you answered <font color=' . $acolor . '>' . $answer_selected . '<font color=black> <br />';
    echo 'the correct answer was ' . $correct . '<br />' ;
    echo '-------------------------------------- <br />' ;
    $x++;
}
echo 'आपके  ' . $score . ' है  ' . $i . ' मे से';
...
if(!isset($_SESSION['resultsaved'])){
$mysqli->query("INSERT INTO result(username,section,correct,incorrect,unanswered,stime,rtime,score,ip) VALUES('$username','1','$r','$s','$t','$starttime','$timer','$score','$ipadr')")or die($mysqli->error);
$_SESSION['resultsaved']="result saved";}
